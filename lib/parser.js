...
HtmlError.prototype.toString = function() {
	return 'Syntax Error: ' + (this.filename ? this.filename + ': ' : '') + 'Line ' + this.line + ', Column ' + this.column + '\n\t' + this.message;
};
...
Parser.prototype._processElementTag = function(o) { // Parameter is the current index in
	if(!this._htmlTag) {
		...
		var attrText = this._decoder.write(this._htmlElementFull.slice(this._tagStart, o)).trim();
	...
	{
		if(!this._tagDone) {
			this._currentAttribute = this._decoder.write(this._htmlElementFull.slice(this._tagStart, o)).trim();
			if(this._currentAttribute.length > 0) { // Make sure there's actually an attribute (none of ="")
				this._htmlAttributes[this._currentAttribute] = this._currentAttribute; // HTML valid
				this._currentAttribute = undefined;
			}
		}
	}
			...
			case EnumParserCharacterCodes.HtmlTagClose:
				if(this._state == EnumParserState.ElementOpen) {
					...
					if(this._parseElement) {
						...
						for(; o < this._htmlElementFull.length; o++) {
							switch(this._htmlElementFull[o]) {
								...
								case EnumParserCharacterCodes.HtmlEqualSign:
									if(this._htmlTag && this._attributeDone && !this._currentAttribute) {
						...
						if(this._callbacks.onclosetag && this._tagDone && this._options.recognizeSelfClosing) {
